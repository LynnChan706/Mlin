...
public:
    ...
    template<typename _T> lMat<T> operator+(lMat<_T>  _m)
    {
        if( (size_m()._c!=_m.size_m()._c) || (size_m()._r!=_m.size_m()._r))
        {
            throw "mat operate + error";
        }
        lMat<T> _mat(size_m()._r,size_m()._c,0);
        for(int i=0; i!=_mat.size_m()._r; ++i)
             for(int j=0; j!=_mat.size_m()._c; ++j)
